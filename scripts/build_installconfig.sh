...
echo "Creating floating IPs for cluster..."
...
echo "  ✓ API Floating IP: ${FLOATINGIP_API}"
echo "  ✓ Ingress Floating IP: ${FLOATINGIP_INGRESS}"
...
if [ "${OPENSHIFT_NEEDS_PROXY}" = "yes" ]; then
  echo "Configuring proxy settings in install-config.yaml..."
  yq eval -i "
...
if [ "${OPENSHIFT_NEEDS_PROXY}" = "yes" ]; then
  yq eval '.pullSecret = "<REDACTED>" | .sshKey = "<REDACTED>" |
    .proxy.httpProxy |= sub(":[^:@]+@"; ":<REDACTED>@") |
    .proxy.httpsProxy |= sub(":[^:@]+@"; ":<REDACTED>@")' "${INSTALL_CONFIG}"
else
  yq eval '.pullSecret = "<REDACTED>" | .sshKey = "<REDACTED>"' "${INSTALL_CONFIG}"
fi
echo ""

echo "Updating /etc/hosts with cluster endpoints..."
...
if [ "${hosts}" != "${old_hosts}" ]; then
    ...
    echo "  ✓ /etc/hosts updated with cluster endpoints"
else
